...
public class CreateApprover {
	...
	protected void handleSaveAction(ActionEvent event) {
		...
		ctrl.createApprover(new User(txtUsername.getText(), txtEmail.getText(), txtPassword.getText(), cbxCountry.getValue()));
		Window.alert("Success", "Approver created");
	...
	private boolean isValid() {
		...
		User user = ctrl.getUserByEmail(txtEmail.getText());
		if(user != null && user.hasRole(Approver.class)) {
			if(msg.length() > 0) {
				msg.append("\r\n");
			}
			msg.append("This email is already related to a Approver user");
		}
